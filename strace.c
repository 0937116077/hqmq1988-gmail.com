...
ptrace_next_event(void)
{
	...
	/*
	 * We wait for new events until wait4() returns 0 (meaning that there's
	...
	for (;;) {
		...
		if (wait_tab_pos >= tcb_wait_tab_size)
			tcb_wait_tab = xgrowarray(tcb_wait_tab,
						  &tcb_wait_tab_size,
						  sizeof(tcb_wait_tab[0]));

		/* Initialising a new wait data structure */
